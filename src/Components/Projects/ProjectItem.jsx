const ProjectItem = ({ name, category, imagePath, videoPath, isWide, isVideo }) => {
  const renderContent = () => {
    if (isVideo && videoPath) {
      ...
      return <div className="project-img" style={{ backgroundImage: `url(${imagePath})` }} />;
    } else {
      // Opzione per gestire l'assenza di un'immagine
      // Puoi renderizzare un placeholder o nulla
      return <div className="project-img-placeholder">Immagine non disponibile</div>;
    }
  };
  ...
  return (
    <div className={`project ${isWide ? 'project-wide' : ''}`}>
      {renderContent()}
      <div className="project-name"><p>{name}</p></div>
